...
class DensityRatioLoss(nn.Module):
    ...
    def forward(self, T_real: torch.Tensor, T_fake: torch.Tensor):
        ...
        if self.loss_type == "dv":
            ...
            # Optional normalization penalty
            if self.rulsif_norm_penalty > 0.0:
                Er_r = r_fake.mean()
                loss = loss + self.rulsif_norm_penalty * (Er_r - 1.0) ** 2
            
            # Compute true density ratio from r_alpha
            # r = (1-α) * r_α / (1 - α * r_α)
            with torch.no_grad():
                true_r_real = (1 - self.rulsif_alpha) * r_real / \
                             (1 - self.rulsif_alpha * r_real).clamp(min=EPS)
                true_r_fake = (1 - self.rulsif_alpha) * r_fake / \
                             (1 - self.rulsif_alpha * r_fake).clamp(min=EPS)
            ...
            metrics.update({
                ...
                "constraint_resid": (r_fake.mean() - 1.0).abs().item() if self.rulsif_norm_penalty > 0 else 0,
            })
